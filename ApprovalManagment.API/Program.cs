...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "ApprovalManagment.API", Version = "v1" });
...
builder.Services.AddDbContext<ApprovalManagmentDbContext>(options => options.UseSqlServer(configuration.GetConnectionString("DefaultConnection")));
...
).AddDefaultTokenProviders().AddEntityFrameworkStores<ApprovalManagmentDbContext>();
